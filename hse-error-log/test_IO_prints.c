...
static void bl2_simple_io_test(void)
{
	...
	NOTICE("BL2 IO TEST: Image source OK\n");
	 ...
	 * Step 2: Open image
	 * -------------------------------------------------- */
	NOTICE("BL2 IO TEST: Opening image handle\n");
	...
	NOTICE("BL2 IO TEST: io_open OK\n");
	NOTICE("BL2 IO TEST: image_handle = 0x%lx\n", image_handle);

	/* --------------------------------------------------
	 * Step 3: Try SEEK (expected to FAIL)
	 * -------------------------------------------------- */
	...
	/* --------------------------------------------------
	 * Step 4: Try WRITE (expected to FAIL)
	 * -------------------------------------------------- */
	NOTICE("BL2 IO TEST: Attempting io_write (0x%08x)\n", write_val);
	...
	/* --------------------------------------------------
	 * Step 5: Try READ (may or may not work depending on backend)
	 * -------------------------------------------------- */
...
close:
	NOTICE("BL2 IO TEST: Closing image handle\n");
