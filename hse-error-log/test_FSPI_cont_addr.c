...
#include <platform_def.h>   /* For NXP_FLEXSPI_ADDR */
	...
	while (bl2_node_info != NULL) {
		...
		if ((bl2_node_info->image_info->h.attr &
			...
			if (plat_setup_done != 0) {
				...
				/* -------------------------------------------------
				 * FlexSPI register access sanity test
				 * Reads MCR0 @ NXP_FLEXSPI_ADDR
				 * -------------------------------------------------
				 */
				val = *(volatile uint32_t *)(NXP_FLEXSPI_ADDR + 0x0);
				NOTICE("BL2: FlexSPI MCR0 = 0x%08x\n", val);
			}
		}
