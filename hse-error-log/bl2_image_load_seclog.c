...
/* 🔐 Secure boot logging */
#include <drivers/secboot_log/secboot_log.h>
...
struct entry_point_info *bl2_load_images(void)
{
	...
	/* 🔐 Initialize secure boot log */
	secboot_log_init();

	/*
	 * Get information about images to load
	 */
	...
	while (bl2_node_info != NULL) {
		...
		if (err != 0) {
			secboot_log_error(bl2_node_info->image_id, err);
			ERROR("BL2: Pre image load failed (%i)\n", err);
		...
		if ((bl2_node_info->image_info->h.attr &
			...
			if (err != 0) {
				...
				ERROR("BL2: Authentication failed for image %u (%i)\n",
		...
		if (err != 0) {
			secboot_log_error(bl2_node_info->image_id, err);
			ERROR("BL2: Post image load failed (%i)\n", err);
			plat_error_handler(err);
		}
	...
	if (bl2_to_next_bl_params->head->ep_info->args.arg0 == 0U) {
