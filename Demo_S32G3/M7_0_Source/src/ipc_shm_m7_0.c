...
#define WAIT_TIMEOUT 0x001000000
...
static uint8 xSemaphoreTake(SemaphoreHandle_t psema, uint32 timeout)
{
	uint32 time = 0;
...
void shm_demo(void)
{
	...
	while (1) {
		...
		if (xSemaphoreTake(shm_event_rx, WAIT_TIMEOUT)) {
			/* get buffer */
			buf = ipc_shm_acquire_buf(IPCF_INSTANCE, app.last_rx_ch,
					MAX_SAMPLE_MSG_LEN);
			if (!buf)
				handle_error(-IPC_SHM_E_NOMEM);
			...
			/* send echo reply to peer */
			err = ipc_shm_tx(IPCF_INSTANCE, app.last_rx_ch, buf,
					ipc_strlen(buf));
			if (err) {
				handle_error(err);
			}
			...
			if (app.rx_count == app.expected_msgs) {
				reply_ctrl_msg(IPCF_INSTANCE, CTRL_CHAN_ID);
			}
		}
	}
