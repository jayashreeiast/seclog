...
typedef uint16_t hseAttrId_t;
    ...
    #define HSE_DISABLE_KEY_USAGE_ATTR_ID   ((hseAttrId_t)33U)  /**< brief SET-ONLY-ONCE-ATTR; Disable key usage until next reset (see #hseDisableKeyUsage_t) */
    ...
    #ifdef HSE_SPT_DICE
    #define HSE_DICE_ATTR_ID                        ((hseAttrId_t)500U)  /**< @brief NVM-RW-ATTR; Specifies the DICE attribute (see #hseDiceAttr_t) */
    #endif /* HSE_SPT_DICE */
...
#ifdef HSE_SPT_DICE
/** @brief   DICE attribute.
 *  @details The DICE attribute must be configured before calling any DICE services or
 *           before calling hseSmrEntryInstallSrv_t service.
 *           @note
 *           - SuperUser rights are needed to configure this attribute.
 *           - It can be read without any restriction.
 *           - This attribute must be configured before installing the SMR for CDI0. The hseSmrEntryInstallSrv_t
 *             service must be alway called when the SMR is updated (the SHA512 of SMR is computed during installation).
 *             Additionally, the SYS-IMG must be published.
 *  */
typedef struct {
    hseKeyHandle_t  keyHandleNvmUDSKeyPair;	/**< @brief The handle to the NVM UDS key pair. */
    uint8_t         cdi0SmrIdx;	            /**< @brief The SMR identifier for the CDI0 key pair generation.
                                                            The SMR must be installed after setting this attribute. */
    hseEccCurveId_t diceEccCurveId;	        /**< @brief The EC curve for the Open-DICE service. Only ECC p256 and p384 curves are supported. */
    hseHashAlgo_t   certSignHashAlgo;	    /**< @brief The hash primitive to be used for signing the CDIn certificates. Only SHA-256, SHA-384 or SHA-512 is supported */
    uint8_t         reserved[57U];          /**< @brief Reserved */
} hseDiceAttr_t;
...
/*==================================================================================================
                                 Disable Key Usage Attribute
==================================================================================================*/
/** @brief   Disable until next reset the usage of a key (SET_ONLY_ONCE_ATTR attribute; refer to #hseAttrId_t).
 *  @details Usecase: To elevate offline execution rights to SU, the host can duplicate the authorization key in another key slot,
 *           and compute the authentication tag over the SU challenge response in the #hseSysAuthorizationRespSrv_t service.
 *           This duplicated key shall have the HSE_KF_USAGE_SIGN flag set and it can be disabled calling this service.
 *
 *           @note: This is a "SET-ONLY-ONCE-ATTR" attribute.
 *           The attribute can only be set from #HSE_INVALID_KEY_HANDLE to a #hseKeyHandle_t type value.
 *           Once the attribute is set, it cannot be set to a different value in the current power cycle.
 */
typedef hseKeyHandle_t hseDisableKeyUsage_t;
