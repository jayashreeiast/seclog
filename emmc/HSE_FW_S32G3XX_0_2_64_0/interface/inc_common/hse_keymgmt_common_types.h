...
#if defined(HSE_SPT_DICE)
#define HSE_KF_USAGE_CDI_SIGN         ((hseKeyFlags_t)1U << 14U)    /**< @brief The private key of UDS and CDIn key pairs can be used only with #hseDiceGenSignCertSrv_t service for signing certificates.
                                                                                This flag is set when DICE ECC keypairs are generated, and cannot be set using the key import service */
#else
#define HSE_KF_USAGE_CDI_SIGN         ((hseKeyFlags_t)0U)
#endif /* HSE_SPT_DICE */
...
#define HSE_KF_USAGE_MASK   \
       (HSE_KF_USAGE_ENCRYPT | HSE_KF_USAGE_DECRYPT | HSE_KF_USAGE_SIGN | HSE_KF_USAGE_VERIFY | HSE_KF_USAGE_EXCHANGE | \
        ...
        HSE_KF_USAGE_XTS_TWEAK | HSE_KF_USAGE_OTFAD_DECRYPT | HSE_KF_USAGE_CDI_SIGN)
